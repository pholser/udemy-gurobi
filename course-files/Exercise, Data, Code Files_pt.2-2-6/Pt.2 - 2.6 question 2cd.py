...
# Constraints: Can't purchase dataset 2 fewer than 0 times
m.addConstr(NumToBuy - sum(x[i] for i in range(DataSets) if i != 1) >= 0)

# Constraints: Can't purchase dataset 2 more than one time
m.addConstr(NumToBuy - sum(x[i] for i in range(DataSets) if i != 1) <= 1)
...
if NumToBuy - sum(x[i].X for i in range(DataSets) if i != 1) > 0.9999:
    print(f"Also purchase dataset 2")
